...
PostArtistRoutes.post(
  ...
  async (c) => {
    ...
    const [counts] = await db.select({count: s.authorMain.uuid}).from(s.authorMain).orderBy(desc(s.authorMain.uuid)).limit(1)
    if(!body.uuid){
      body.uuid = counts.count + 1
    }
    const returnResponse = await db.insert(s.authorMain).values(body).returning()
