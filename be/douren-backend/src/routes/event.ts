...
export const trpcEventRoute = router({
  ...
  getEventId: publicProcedure.input(eventNameInputParams).output(zodSchema.event.SelectSchema).query(async (opts) => {
    ...
    const db = initDB()
    const [data] = await db.select().from(s.event).where(eq(s.event.name, eventName))
    return data;
  })
})
...
const EventRoute = new Hono<{ Bindings: BACKEND_BINDING }>()
  .get("/:eventId/artist", async (c) => {
    ...
    const returnObj = await EventArtistDao.Fetch({page, search, sort, searchTable, tag, eventId})
