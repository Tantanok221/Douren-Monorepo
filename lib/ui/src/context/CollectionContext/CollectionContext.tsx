...
function reducer(draft:eventArtistBaseSchemaType[] | null , {action,keys: key,data}: ActionType) {
	switch(action){
		case 'add': {
			if(data && draft){
				draft.push(data)
...
export const CollectionContextProvider = ({children,keys}: Props) => {
	...
	const [collection, dispatch] =  useImmerReducer(reducer,item as eventArtistBaseSchemaType[]| null)
	return (
